...
static void ggml_compute_forward_swiglu_f32(
    ...
    // expert-cache v3: rows whose gate/up matvecs ran fused on the GPU arrive
    // pre-activated; thread 0 scatters them into dst inside glu_hits and every
    // thread skips them here
    unsigned long long ec3_skip = 0;
    if (src1 && ggml_expert_cache_v3.glu_hits && nr <= 64) {
        ec3_skip = ggml_expert_cache_v3.glu_hits(src0->data, src1->data,
